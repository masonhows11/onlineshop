...
<form action="{{ route('search.category',['slug' => request()->slug]) }}" method="get">
            ...
            @forelse( $brands as $brand )
                <div class="form-check mb-3">
                    <input type="checkbox"
                           ...
                           name="brands[]" value="{{ $brand->id }}" class="form-check-input"
                           id="brand-{{ $brand->id }}">
                    <label class="form-check-label font-12" for="brand-{{ $brand->id }}">{{ $brand->title_persian }}</label>
                </div>
            @empty
                <div class="form-check mb-3">
